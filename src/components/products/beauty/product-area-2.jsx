...
const tabs = ["Tất cả", "Xu hướng", "Chăm sóc da", "Mỹ phẩm"];

const ProductAreaTwo = () => {
  const [activeTab, setActiveTab] = useState(tabs[0]);
  const { data: products, isError, isLoading } = useGetProductTypeQuery({ type: "beauty" });
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    } else if (activeTab === "Chăm sóc da") {
      product_items = products.data.filter((p) => p.category.name === "Discover Skincare");
    } else if (activeTab === "Mỹ phẩm") {
      product_items = products.data.filter((p) => p.category.name === "Awesome Lip Care");
    }
        ...
        <div className="row align-items-end">
          <div className="col-xl-6 col-lg-6">
            <div className="tp-section-title-wrapper-3 mb-45 text-center text-lg-start">
              <span className="tp-section-title-pre-3">Nổi bật tuần này</span>
              <h3 className="tp-section-title-3">Lựa chọn chất lượng cho bạn</h3>
            </div>
          </div>
          <div className="col-xl-6 col-lg-6">
